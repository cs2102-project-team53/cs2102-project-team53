...
BEGIN
    -- **check for fever before doing anything. If no fever do nothing
...
BEGIN
   ...
   WITH num_declarations AS ( 
		 ...
		 GROUP BY h.eid
	     ORDER by num_days asc --**Redundant since num_days in constant, if you meant num_declared then you are ordering in the next statement anyway so can remove this
	   
   ) -- asc means eid with most temps declared first
